...
for f in $in_dir/{utt2spk,spk2utt,wav.scp}; do
  if [ ! -f $f ]; then
...
#fi
#if [ $(cat $dir/temp/utt2len | wc -l) -ne $(cat $in_dir/utt2spk | wc -l) ]; then
#  echo "utt2spk and utt2spk files have mismatched lengths";
#  exit 1;
#fi
...
# and pipe it into a perl script that outputs the segments file.
  awk '{print $2}' $in_dir/utt2spk | paste $dir/temp/utt2len -  | \
   paste -  <(awk '{print $2}' $in_dir/utt2spk) | perl -e '
  ...
  open(utt2spk, ">$out_dir/utt2spk") || die "opening segments file $out_dir/utt2spk";
  while(<STDIN>) {
    ...
    if ($len <= $max_utt_len) {
      ...
      for ($n = 1; $n <= $num_split; $n++) {
         ...
         print utt2spk "${utt}-$n_text $language\n";
      }
    }
  }
  close(SEGMENTS)||die; close(UTT2SPK)||die; close(utt2spk)||die; ' $max_utt_len $dir
fi
